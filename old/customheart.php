...
$date_two = date('Y-m-d',strtotime($_POST['to']));
    ...
    $sql = "select * from heartrate where clientID = '$clientID' and dateandtime between '$date_one' and '$date_two';";
    ...
    echo json_encode(['heart' => $full]);
        ...
        for ($i=0; $i <= $diff->format('%m'); $i++) { 
            $avgArr['avg'] = getArr($from[$i],$to[$i],$clientID);
            $avgArr['date'] = $i+1;
            $avgJson[] = $avgArr;
        }
