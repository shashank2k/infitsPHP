...
$from = $_POST['from'];
$to = $_POST['to'];
$date_one = date('Y-m-d', strtotime($from));
$date_two = date('Y-m-d',strtotime($to));
    // echo $date_one;
    // echo $date_two;
    ...
    $sql = "select hrsSlept,sleeptime from sleepTracker where clientID = '$clientID' and sleeptime between '$date_one' and '$date_two';";
    ...
    echo json_encode(['sleep' => $full]);
        ...
        for ($i=0; $i <= $diff->format('%m'); $i++) { 
            $avgArr['hrsSlept'] = getArr($from[$i],$to[$i]);
            $avgArr['date'] = $i+1;
            $avgJson[] = $avgArr;
        }
        echo json_encode(['sleep' => $avgJson]);   
    }
?>